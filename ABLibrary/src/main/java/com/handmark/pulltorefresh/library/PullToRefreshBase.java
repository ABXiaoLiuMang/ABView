...
//		}
	...
	protected LoadingLayoutBase createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
        LoadingLayoutBase layout = mLoadingAnimationStyle.createLoadingLayout(context, mode, attrs);
	 ...
	 * 显示加载成功，延时1秒恢复
	 */
	private void onRefreshSucc() {
		if (mMode.showHeaderLoadingLayout()) {
			mHeaderLayout.onRefreshSucc();
		}
		if (mMode.showFooterLoadingLayout()) {
			mFooterLayout.onRefreshSucc();
		}
	}
	...
	public static enum AnimationStyle {
        ...
        LoadingLayoutBase createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
			switch (this) {
			case ROTATE:
			default:
